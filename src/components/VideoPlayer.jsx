...
const Player = ({miniPlayer}) => {
  ...
  function onMouseStill(callback, delay = 2000) {
    ...
    function onMouseMove() {
      ...
      timeoutId = setTimeout(callback, delay); // Set new timeout
    }
    ...
    document.addEventListener('mouseleave', onMouseLeave); // Listen for mouse leave events
  
    // Return a cleanup function to remove event listeners when no longer needed
  ...
  useEffect(() => {
    ...
    // To stop listening, call the cleanup function
  ...
  useEffect(() => {
    ...
    return () => {
      document.onfullscreenchange = null; // Cleanup on unmount
    };
  }, []);
  ...
  useEffect(() => {
    const handleKeyDown = (event) => {
      switch (event.key) {
        ...
        case 'ArrowUp':
          event.preventDefault();
          setVolume((prev)=>parseFloat((prev+0.1)>1?1:(prev+0.1))); // Increase volume by 10%
          break;
        case 'ArrowDown':
          event.preventDefault();
          setVolume((prev)=>parseFloat((prev-0.1)<0?0:(prev-0.1))); // Decrease volume by 10%
          break;
        case 'ArrowRight':
          event.preventDefault();
          handleForward() // Forward 10 seconds
          break;
        case 'ArrowLeft':
          event.preventDefault();
          handleBackward() // Backward 10 seconds
          break;
        case 'm':
        case 'M':
          event.preventDefault();
  ...
  return (
    <div className={`flex flex-row w-full gap-6  ${fullScreen?"":"p-6"}`}>
        <div onMouseOver={()=>setHover(true)} onMouseLeave={()=>setHover(false)} className={`relative w-full aspect-video rounded-lg overflow-clip `}>
      <video controls={false}  className=' w-full aspect-video   ' src={src} ref={playerRef} onTimeUpdate={handleTimeUpdate} />
      
      <div className=' absolute top-0 left-0 p-2 z-20 w-full flex flex-row items-center justify-between rounded-lg overflow-clip '  style={{opacity:!mouseStill&&hover?0.7:0}}>
      ...
      {miniPlayer?
      (<></>)
      :<div className=' flex flex-row w-full items-center justify-between px-6 py-3 gap-5 ' >
      ...
      <h3 className=' text-xs font-light'>{numberToString(currentTime)}</h3>
        ...
        {/*<h3 className={`text-lg font-bold ${volHover?"":" translate-x-full"} transition-all`}>Volume</h3> */}
